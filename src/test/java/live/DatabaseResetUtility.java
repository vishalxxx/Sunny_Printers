...
public class DatabaseResetUtility {
	...
	public void runReset() throws Exception {
		System.out.println("Starting Database Reset Utility (TEST environment only)...");
		
		// Load TEST credentials — never use production
		TestEnvironment.load();
		if (!TestEnvironment.isSupabaseConfigured()) {
			System.out.println("TEST Supabase credentials not found. Skipping database reset.");
			return;
		}
		...
		// Create isolated SQLite database for reset operations
		String testDbUrl = TestDatabaseHelper.createIsolatedDb("DatabaseReset");
		DBConnection.setTestDatabaseUrl(testDbUrl);
		DBConnection.setGlobalTestDatabaseUrl(testDbUrl);

		try {
			SupabaseRestClient http = new SupabaseRestClient(supabaseUrl, anonKey);
				...
				try (Statement stmt = conn.createStatement()) {
					...
					for (String table : TABLES_TO_WIPE) {
						...
						try {
							...
							HttpRequest.Builder req = HttpRequest.newBuilder(URI.create(supabaseUrl + "/rest/v1/" + table + "?" + filter))
									.DELETE()
					...
					try {
						...
						HttpRequest.Builder req = HttpRequest.newBuilder(URI.create(supabaseUrl + "/rest/v1/number_sequences?sequence_key=not.is.null"))
					...
					for (String table : TABLES_TO_WIPE) {
						...
						// Verify remote
						try {
							HttpRequest.Builder req = HttpRequest.newBuilder(URI.create(supabaseUrl + "/rest/v1/" + table + "?select=*"))
									.GET()
									.header("apikey", anonKey)
									.header("Authorization", "Bearer " + anonKey)
