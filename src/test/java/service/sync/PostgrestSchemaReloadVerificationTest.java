...
public class PostgrestSchemaReloadVerificationTest {
    ...
    public void runVerification() throws Exception {
        System.out.println("Starting Final PGRST204 Verification...");
        ...
        SupabaseSettings settings = new repository.SupabaseSettingsRepository().load();
        String anonKey = settings.getAnonKey();
        if (anonKey == null || anonKey.isBlank()) {
            anonKey = "fake-anon-key";
        }
        SupabaseRestClient captureClient = new SupabaseRestClient(optClient.get().restV1Base().replace("/rest/v1/", ""), anonKey) {
            @Override
