...
public class SchemaAndSyncChecker {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("âœ” Successfully fetched remote schema with " + definitions.size() + " tables.");
            ...
            try (Connection conn = DBConnection.getConnection()) {
                ...
                for (String tableName : tablesToCheck) {
                    ...
                    if (!localExists) {
                        System.out.println("âŒ Local: Table DOES NOT exist in SQLite!");
                        totalMismatches++;
                        continue;
                    }
                    System.out.println("âœ” Local: Table exists.");

                    // Check if table exists on remote
                    JsonObject remoteTable = definitions.getAsJsonObject(tableName);
                    if (remoteTable == null) {
                        System.out.println("âŒ Remote: Table DOES NOT exist in Supabase!");
                        totalMismatches++;
                        continue;
                    }
                    System.out.println("âœ” Remote: Table exists.");
                    ...
                    if (missingOnRemote.isEmpty() && typeMismatches.isEmpty()) {
                        System.out.println("âœ” Schema Match: All local columns exist on remote with compatible types.");
                    } else {
                        if (!missingOnRemote.isEmpty()) {
                            System.out.println("âš ï¸ Columns missing on remote: " + missingOnRemote);
                            totalMismatches += missingOnRemote.size();
                        }
                        if (!typeMismatches.isEmpty()) {
                            System.out.println("âš ï¸ Column type mismatches: ");
                ...
                if (totalMismatches == 0) {
                    System.out.println("ðŸŽ‰ SCHEMA VERIFICATION SUCCESSFUL! No mismatch found.");
                } else {
                    System.out.println("âŒ SCHEMA VERIFICATION COMPLETED with " + totalMismatches + " issues/mismatches.");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
