...
public class E2eFlowTest {
    ...
    public void testOnlineE2EFlow() throws Exception {
        ...
        AtomicDB.runVoid(con -> {
            exec(con,
                    "INSERT INTO invoice_master (uuid, invoice_no, client_uuid, client_name, invoice_date, period_from, period_to, amount, paid_amount, due_amount, payment_status, type, status, is_void, document_series, sync_status, sync_version, is_deleted, is_active, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,0,'GST_INVOICE','PENDING',1,0,1,datetime('now'),datetime('now'))",
                    invoiceUuid, invoiceNo, clientUuid, "Online Business", today.toString(),
                    today.withDayOfMonth(1).toString(), today.toString(), 1000.0, 0.0, 1000.0, "UNPAID", "DATE_RANGE",
                    "FINAL");
            exec(con,
                    "UPDATE jobs SET invoice_uuid=?, status='Invoiced', sync_status='PENDING', sync_version=COALESCE(sync_version,0)+1, updated_at=datetime('now') WHERE uuid=?",
                    invoiceUuid, jobUuid);
            exec(con,
                    "INSERT INTO invoice_job_mapping (uuid, invoice_uuid, job_uuid, sync_status, created_at, updated_at) VALUES (?,?,?,'PENDING',datetime('now'),datetime('now'))",
                    mappingUuid, invoiceUuid, jobUuid);
        });

        // 5. Add Payment & Allocation (type = Payment)
        AtomicDB.runVoid(con -> {
            exec(con,
                    "INSERT INTO payments (uuid, client_uuid, amount, payment_date, method, type, sync_status, sync_version, is_deleted, is_active, created_at, updated_at) VALUES (?,?,?,?,'Cash','Payment','PENDING',1,0,1,datetime('now'),datetime('now'))",
                    paymentUuid, clientUuid, 600.0, today.toString());
            exec(con,
                    ...
                    paymentAllocUuid, paymentUuid, invoiceUuid, 600.0);
            exec(con,
                    "UPDATE invoice_master SET paid_amount=600.0, due_amount=400.0, payment_status='PARTIAL PAID', sync_status='PENDING', updated_at=datetime('now') WHERE uuid=?",
                    invoiceUuid);
        });
        ...
        AtomicDB.runVoid(con -> {
            exec(con,
                    "INSERT INTO payments (uuid, client_uuid, amount, payment_date, method, type, sync_status, sync_version, is_deleted, is_active, created_at, updated_at) VALUES (?,?,?,?,'Cash','Refund','PENDING',1,0,1,datetime('now'),datetime('now'))",
                    refundUuid, clientUuid, -100.0, today.toString());
            exec(con,
                    "INSERT INTO payment_allocations (uuid, payment_uuid, invoice_uuid, allocated_amount, sync_status, created_at, updated_at) VALUES (?,?,?,?,'PENDING',datetime('now'),datetime('now'))",
                    refundAllocUuid, refundUuid, invoiceUuid, -100.0);
            exec(con,
                    "UPDATE invoice_master SET paid_amount=500.0, due_amount=500.0, payment_status='PARTIAL PAID', sync_status='PENDING', updated_at=datetime('now') WHERE uuid=?",
                    invoiceUuid);
        });
        ...
        AtomicDB.runVoid(con -> {
            exec(con,
                    "INSERT INTO invoice_adjustments (uuid, invoice_uuid, type, note_no, amount, reason, date, sync_status, sync_version, is_deleted, is_active, created_at, updated_at) VALUES (?,?,?,?,?,?,?,'PENDING',1,0,1,datetime('now'),datetime('now'))",
