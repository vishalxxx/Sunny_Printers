...
public class InvoiceBuilderService {
	...
	public Invoice buildInvoiceForClientByJobs(int clientId, String clientName, String businessName,
		...
		String sql = "SELECT " + "    j.id        AS job_id, " + "    j.job_no    AS job_no, "
				+ "    j.job_date  AS job_date, " + "    j.job_title AS job_name, " + "    ji.description, "
				+ "    ji.amount, " + "    ji.type, " + "    ji.sort_order " + "FROM jobs j "
				+ "JOIN job_items ji ON ji.job_id = j.id " + "WHERE j.client_id = ? " + "  AND j.id IN (" + placeholders
	...
	public Map<String, Invoice> buildMonthlyInvoicesForAllClients(int year, int month) {
		...
		try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			while (rs.next()) {
				...
				Invoice invoice = buildInvoiceForMonthlyClient(clientId, clientName, businessName, fromDate, toDate,
						con);
	...
	public Invoice buildInvoiceForMonthlyClient(int clientId, String clientName, String businessName,
			LocalDate fromDate, LocalDate toDate, Connection con) {
