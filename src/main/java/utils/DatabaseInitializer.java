...
public class DatabaseInitializer {
    ...
    public static void initialize() throws Exception
 {
        ...
        try (Connection conn = DriverManager.getConnection("jdbc:sqlite:database/sunnyprinters.db");
            ...
            try {
                stmt.execute("""
                    INSERT INTO bank_details (bank_name, account_holder_name, account_no, branch_ifsc, is_default, is_active)
                    SELECT 'INDIAN OVERSEAS BANK', 'SUNNY PRINTER', '15980200000000780', 'PITAMPURA & IOBA0001598', 1, 1
            ...
            // ================== SUPABASE SETTINGS (optional cloud DB / future sync) ==================
            stmt.execute("""
            ...
            stmt.execute("""
                    ...
                    (id, supabase_url, anon_key, auth_email, auth_password)
                    VALUES (1, '', '', '', '');
                    """);
    ...
    public static void seedDefaultHsnSacRows(Statement stmt) {
        ...
        for (String[] r : rows) {
            try {
                stmt.execute("""
                        INSERT INTO hsn_sac_master (item_type, item_name, keyword, code_type, hsn_sac, gst_rate, unit_default, description, is_favorite, is_active)
                        SELECT '%s', '%s', '', 'HSN', '—', 0.18, 'PCS', '', 0, 1
                        WHERE NOT EXISTS (
    ...
    private static void migrateHsnSacMasterIfNeeded(Connection conn) throws java.sql.SQLException {
        ...
        try (Statement st = conn.createStatement()) {
            ...
            st.executeUpdate("""
                    ...
                    """);
            st.executeUpdate("""
                    INSERT INTO hsn_sac_master (id, item_type, item_name, keyword, code_type, hsn_sac, gst_rate, unit_default, description, is_favorite, is_active, created_at)
                    SELECT id, item_type,
